...
Remember from the discussion in [Class 2](https://github.com/nyu-pl-fa22/class02) that the memory used to
...
(e.g. real-time applications such as embedded controllers, multi-media
...
is parameterized by a type `T` (the type of values to which a `Ptr` smart pointer
points)`.

Each instance of `Ptr<T>` has two fields:
...
C++ does not have fully automatic memory management. 

The destructor of a class `A` is denoted by `~A`.
...
If the programmer does not define these three methods (copy
constructor, assignment operator, destructor) explicitly in a class,
then the C++ compiler will add them automatically. The semantics of
the default implementations are:

* Destructor - Call the destructors of all of the object's class-type
  ...
  automatically call the destructor `~A` for the `A` instance stored
  in field `a` because this `A` instance is directly embedded within
  the data layout of the `B` instance. So `a` will be deleted along
  with the enclosing `B` instance. However, the destructor of the `A`
...
* Destructor 
  ...
  When a `Ptr` is destroyed, its destructor is called. The
...
* Assignment Operator
  ...
  object and counter, which would lead to a use-after-free error when we
...
One way to realize this is by implementing a more sophisticated static
